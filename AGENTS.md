# CTF Agent Browser Use Integration

This repository wires Browser Use into the solver swarm as an opt-in backend with model specs under `browser-use/*`.

- Use `browser-use/bu-latest` when a challenge genuinely requires a real browser.
- Browser Use runs the browser on the host, not inside the Docker sandbox.
- The Docker sandbox is still the source of truth for shell commands, local file analysis, exploit scripts, and flag submission.
- Browser downloads are written to the sandbox workspace host directory and become visible inside the container at `/challenge/workspace`.
- `BROWSER_USE_API_KEY` is required for this backend.
- Browser Use is **not** part of `DEFAULT_MODELS`; it must be requested explicitly via `--models`.
